...
const getEvents = async (req, res) => {
  ...
  const promises = events.map(async event => {
    const panitia = await queryAsync(`select id as id_panitia, nama_devisi, id_devisi from panitia where id_event=${event.id}`)
    const ceklist = await queryAsync(`select id as id_ceklist, nama_ceklist from ceklist where id_event=${event.id}`)
    const agenda = await queryAsync(`select id as id_anggaran, nama_section from anggaran where id_event=${event.id}`)
    const anggaran = await queryAsync(`select id as id_anggaran, nama_section from anggaran where id_event=${event.id}`)
    
    event.panitia = panitia
    event.ceklist = ceklist
    event.agenda = agenda
